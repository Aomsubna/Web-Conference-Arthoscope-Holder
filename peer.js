...
// let port = new SerialPort('COM5', { baudRate: 9600, 
//     databits: 8, 
//     parity: 'none', 
//     stopBits: 1, 
//     flowControl: false });
        ...
        btnShareScreen.onclick = event => {
            ...
            navigator.mediaDevices.getDisplayMedia(constraints)
                .then(stream => {
                    ...
                    var localScreen = createVideo2('my-screen');
...
function createOfferer(peerUsername, localScreenSharing, remoteScreenSharing, receiver_channel_name){
    ...
    if(!localScreenSharing && !remoteScreenSharing){
        ...
        remoteVideo = createVideo2(peerUsername + '-screen');
        setOnTrack(peer, remoteVideo);
        console.log('Remote video source: ', remoteVideo.srcObject);
...
function createAnswerer(offer, peerUsername, localScreenSharing, remoteScreenSharing, receiver_channel_name){
    ...
    if(!localScreenSharing && !remoteScreenSharing){
        ...
        // set remote video
        var remoteVideo = createVideo2(peerUsername + '-screen');
        // and add tracks to remote video
        setOnTrack(peer, remoteVideo);
...
function createVideo2(peerUsername){
...
setInterval(()=>{
    ...
    .then(data => {
      ...
      document.getElementById("enc1").innerHTML = data[0];
      document.getElementById("enc2").innerHTML = data[1];
      document.getElementById("enc3").innerHTML = data[2];
      document.getElementById("enc4").innerHTML = data[3];
      document.getElementById("enc5").innerHTML = data[4];
      document.getElementById("enc6").innerHTML = data[5];
      document.getElementById("enc7").innerHTML = data[6];
    })  
},1000)